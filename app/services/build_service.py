...
from app.schemas.build import BuildInfo, BuildStatus, BuildLog, LogType
...
class BuildService:
    ...
    async def _execute_build(
        ...
        try:
            ...
            stdout_task = asyncio.create_task(
                self._read_stream(process.stdout, build_info, LogType.STDOUT)
            )
            stderr_task = asyncio.create_task(
                self._read_stream(process.stderr, build_info, LogType.STDERR)
            )
    ...
    async def _schedule_cleanup(self, workspace_id: str, build_info: BuildInfo):
        ...
        if (workspace_id in self.build_processes and 
            self.build_processes[workspace_id] is build_info):
            del self.build_processes[workspace_id]
            logger.info(f"Cleaned up build info for workspace {workspace_id}")

    def get_build_status(self, workspace_id: str) -> Optional[Dict[str, Any]]:
        ...
        return {
            ...
            "duration": (
                (build_info.end_time or time.time()) - build_info.start_time
            )
        }
    ...
    def get_all_build_statuses(self) -> Dict[str, Dict[str, Any]]:
        return {
            workspace_id: self.get_build_status(workspace_id)
            for workspace_id in self.build_processes
        }
